...
/* ---------------------------------------------------------
   DESIGN TOKENS — "Anchor" · warm editorial cream + one
   rationed terracotta accent. Terracotta appears ONLY on the
   primary Save action (Save Invoice button + its floating
   twin) — everywhere else that needs emphasis (selected bank,
   toggles, active states, dark bars) uses Ink instead, per the
   doc's "never introduce a second chromatic accent" rule.
   Cards are flat 4px radius with no shadow; only Pure White
   "product card" surfaces (item rows, bank tiles) get the
   thin paper-grain shadow.
--------------------------------------------------------- */
const token = {
  bg: "#e3d7cd",          // Cream Canvas — page background
  card: "#f0e9e5",        // Warm White — section/card surface, flat, no shadow
  cardShadow: "none",
  surface: "#ffffff",     // Pure White — elevated "product card" surfaces
  surfaceShadow:
    "0px 1px 1px 0px rgba(0,0,0,0.06), 0px 2px 1px 0px rgba(0,0,0,0.04), 0px 4px 1px 0px rgba(0,0,0,0.01)",
  hover: "#f5f2f0",       // Paper White — hover fill
  border: "#c9c9c7",      // Sand Border
  borderStrong: "#c9c9c7",
  ink: "#1e1e1e",
  inkSoft: "#767676",     // Stone Text
  inkFaint: "#c9c9c7",
  pureBlack: "#1e1e1e",   // no true destructive color in this system — Ink stands in
  darkGround: "#1e1e1e",
  accent: "#ee884f",      // Terracotta — rationed to the single primary action only
  glow: "none",
  glowStrong: "none",
  heroGradient: "linear-gradient(180deg, #e8a87a 0%, #f0c9a3 40%, #f0e9e5 100%)",
};

const fontBody = "'Archivo', 'Inter', ui-sans-serif, system-ui, sans-serif"; // single typeface system
const fontDisplay = fontBody; // Anchor uses one typeface throughout — no separate display face
...
function GhostButton({ children, onClick, danger, small }) {
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-1.5 rounded-full font-semibold uppercase tracking-wide transition-colors ${
...
function PrimaryButton({ children, onClick, full }) {
  ...
  return (
    <button
      ...
      onMouseEnter={() => setHover(true)}
      onMouseLeave={() => setHover(false)}
      className={`inline-flex items-center justify-center gap-2 rounded-full text-sm font-semibold tracking-wide px-6 py-2.5 transition-all ${
        full ? "w-full" : ""
      }`}
      style={{
        fontFamily: fontBody,
  ...
  return (
    <button
      ...
      style={{
        width: 32,
        height: 32,
        border: `1px solid ${danger && hover ? token.pureBlack : active ? token.ink : hover ? token.borderStrong : token.border}`,
        color: danger && hover ? token.pureBlack : active ? "white" : hover ? token.ink : token.inkSoft,
        background: danger && hover ? "#f0f0ef" : active ? token.ink : "white",
...
function ItemRow({ item, index, isFirst, onChange, onInsertAbove, onInsertBelow, onMoveUp, onMoveDown, onDuplicate, onDelete }) {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      >
        {/* Hanging delete — Ink fill (this system rations its one accent
            color to the primary Save action, so delete stays neutral) */}
        <button
          onClick={onDelete}
            ...
            {sections.map((s) =>
              s.type === "group" ? (
                ...
                >
                  ...
                  <div className="px-1 py-2.5 flex flex-col gap-4" style={{ background: "#fbfcfa" }}>
                    {s.items.map((it) => (
                      <ItemRow
                        key={it.id}
                        item={it}
