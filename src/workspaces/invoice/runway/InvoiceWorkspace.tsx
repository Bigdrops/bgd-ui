...
export default function InvoiceWorkspace() {
  ...
  const handleSave = useCallback(() => console.log('Save Runway:', data), [data])
  ...
  return (
    <div className="rw-root" style={{ overflowX: 'hidden' }}>
      {confirmClear && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.4)', padding: '16px' }} onClick={() => setConfirmClear(false)}>
          <div className="rw-card" style={{ maxWidth: '380px', width: '100%', padding: '24px' }} onClick={e => e.stopPropagation()}>
            <div style={{ fontSize: '18px', fontWeight: 600, marginBottom: '8px' }}>Clear all data?</div>
            <p style={{ fontSize: '14px', color: '#61594a', lineHeight: 1.5, marginBottom: '20px' }}>This will remove all items, groups, charges, and notes.</p>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
              <button type="button" onClick={() => setConfirmClear(false)} className="rw-btn rw-btn-sm">Cancel</button>
              <button type="button" onClick={handleClear} className="rw-btn rw-btn-sm rw-btn-danger">Clear All</button>
            </div>
          </div>
        </div>
      )}
      <div style={{ maxWidth: '640px', margin: '0 auto', padding: '16px', minHeight: '100vh' }}>
        <div className="rw-card" style={{ marginBottom: '16px', padding: '20px' }}>
          <InvoiceHeader data={data} onUpdate={update} />
        </div>
        <div className="rw-toolbar">
          <button type="button" onClick={handleSave} className="rw-btn">Save</button>
          <button type="button" onClick={() => setConfirmClear(true)} className="rw-btn rw-btn-danger">Clear All</button>
        </div>
        ...
        <div style={{ display: 'flex', gap: '10px', marginTop: '20px', marginBottom: '40px' }}>
          <button type="button" onClick={handleSave} className="rw-btn rw-btn-primary" style={{ flex: 1, fontSize: '16px', padding: '14px' }}>Save Document</button>
        </div>
      </div>
