...
export interface LineItem {
  id: string; number: number; description: string; subDescription: string
  quantity: number; unit: UnitType; unitPrice: number; total: number
}
...
export interface CommercialSettings {
  paymentTerms: string; discount: Discount; vat: Vat; wht: Wht; charges: AdditionalCharge[]
}
...
export interface DocumentTotals {
  ...
  taxedCharges: number; nonTaxedCharges: number; grandTotal: number; amountInWords: string
}

export interface InvoiceData {
  invoiceNumber: string; title: string; client: Client | null; referenceNumber: string
  issueDate: string; dueDate: string; groups: LineGroup[]; standaloneItems: LineItem[]
  commercial: CommercialSettings; additional: AdditionalInfo
}
