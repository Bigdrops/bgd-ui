            ...
            <div>
              ...
              {showSub && (
                <textarea
                  ...
                  onChange={(e: ChangeEvent<HTMLTextAreaElement>) => onChange({ ...item, subDescription: e.target.value })}
...
export default function InvoiceWorkspace() {
  ...
  const [sections, setSections] = useState<Section[]>([
    {
      ...
      name: "Design Phase",
      items: [
        { type: "item", id: nextId(), description: "Homepage Design", qty: 1, price: 1200 },
        { type: "item", id: nextId(), description: "Dashboard UI", qty: 2, price: 850 },
        { type: "item", id: nextId(), description: "Brand Identity & Logo", qty: 1, price: 600 },
      ],
    },
  ...
  const [charges, setCharges] = useState<Charge[]>([{ id: nextId(), label: "Shipping", value: 25, taxable: true }]);
  ...
  const insertBelowItem = (itemId: string) => {
    setSections((prev) => {
      const next: Section[] = [];
      for (const s of prev) {
        if (s.type === "item") {
  ...
  const insertAboveItem = (itemId: string) => {
    setSections((prev) => {
      const next: Section[] = [];
      for (const s of prev) {
        if (s.type === "item") {
      ...
      <div className="max-w-4xl mx-auto px-1.5 sm:px-4 pt-4 sm:pt-6">
        ...
        <Card title="Commercial Terms">
          ...
          <Collapsible label="Discount">
            <div className="grid grid-cols-2 gap-2">
              <Field label="Value">
                <TextInput type="number" min={0} step={0.01} value={discount.value} onChange={(v) => setDiscount({ ...discount, value: Number(v) || 0 })} />
              </Field>
              <Field label="Type">
          ...
          <Collapsible label="WHT">
            <div className="grid grid-cols-2 gap-2">
              <Field label="Rate">
                <TextInput type="number" min={0} step={0.01} value={wht.rate} onChange={(v) => setWht({ ...wht, rate: Number(v) || 0 })} />
              </Field>
              <Field label="Unit">
              ...
              {charges.map((c) => (
                <div key={c.id} className="flex flex-col gap-1.5">
                  <div className="grid gap-2 items-end" style={{ gridTemplateColumns: "1fr 1fr auto" }}>
                    ...
                    <Field label="Value">
                      <TextInput type="number" min={0} step={0.01} value={c.value} onChange={(v) => setCharges((cs) => cs.map((x) => (x.id === c.id ? { ...x, value: Number(v) || 0 } : x)))} />
                    </Field>
