              ...
              s.type === "group" ? (
                <div key={s.id} className="rounded-2xl overflow-hidden" style={{ border: `2.5px solid ${token.ink}` }}>
                  <div className="flex items-center gap-2 px-3 py-2.5 flex-wrap" style={{ background: token.ink }}>
                    <input value={s.name} onChange={(e: ChangeEvent<HTMLInputElement>) => renameGroup(s.id, e.target.value)} className="flex-1 min-w-[100px] bg-transparent outline-none text-white placeholder-white/50" style={{ fontFamily: fontDisplay, fontWeight: 500, fontSize: 16 }} />
                    ...
                    <span style={{ fontFamily: fontBody, fontWeight: 600, color: token.green }}>${money(s.items!.reduce((a: number, it: Section) => a + calcRowTotal(Number(it.qty) || 0, Number(it.price) || 0), 0))}</span>
              ...
              {charges.map((c) => (
                <div key={c.id} className="flex flex-col gap-1.5">
                  ...
                  <label className="flex items-center gap-2 text-[12px] font-medium" style={{ color: token.ink }}>
                    <input type="checkbox" checked={c.taxable} onChange={(e: ChangeEvent<HTMLInputElement>) => setCharges((cs) => cs.map((x) => (x.id === c.id ? { ...x, taxable: e.target.checked } : x)))} className="w-[16px] h-[16px]" style={{ accentColor: token.green }} />
